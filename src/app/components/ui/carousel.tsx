    ...
    return (
      <CarouselContext.Provider value={{ carouselRef, api, opts, orientation, scrollPrev, scrollNext, canScrollPrev, canScrollNext }}>
        <div
          ...
          className={cn("relative", orientation === "horizontal" ? "carousel-horizontal" : "carousel-vertical", className)}
          role="region"
...
const CarouselPrevious = React.forwardRef<HTMLButtonElement, React.ComponentProps<typeof Button>>(({ className, variant = "outline", size = "icon", ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant={variant}
      size={size}
      className={cn(className, orientation === "horizontal" ? "absolute left-2 top-1/2 -translate-y-1/2" : "absolute top-2 left-1/2 -translate-x-1/2 rotate-90")}
...
const CarouselNext = React.forwardRef<HTMLButtonElement, React.ComponentProps<typeof Button>>(({ className, variant = "outline", size = "icon", ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant={variant}
      size={size}
      className={cn(className, orientation === "horizontal" ? "absolute right-2 top-1/2 -translate-y-1/2" : "absolute top-2 left-1/2 -translate-x-1/2 rotate-90")}
